        ...
        if ( !isset( $bslm_fs ) ) {
            ...
            if ( !file_exists( $freemius_start ) ) {
                ...
                add_action( 'admin_notices', function () {
                    ...
                    echo esc_html__( 'BeeClear Smart Link Manager Premium: Missing Freemius SDK. Please re-install the plugin package (vendor/start.php not found).', 'beeclear-smart-link-manager-premium' );
                    echo '</p></div>';
                } );
                return $bslm_fs;
            }
...
if ( !class_exists( 'BeeClear_ILM', false ) ) {
    class BeeClear_ILM {
        ...
        const OPT_INDEX = 'beeclear_ilm_index';

        // compiled internal rules index
        const OPT_EXT_RULES = 'beeclear_ilm_external_rules';

        // external rules (UI table)
        const OPT_LINKMAP = 'beeclear_ilm_linkmap';

        // runtime link counters (internal)
        const OPT_EXTERNAL_MAP = 'beeclear_ilm_external_map';

        // scan results for external linking
        const OPT_OVERVIEW_SCAN = 'beeclear_ilm_overview_scan';

        // background scan state for overview
        const OPT_OVERVIEW_SCAN_SUMMARY = 'beeclear_ilm_overview_scan_summary';

        // last overview scan summary
        const OPT_ACTIVITY_LOG = 'beeclear_ilm_activity_log';
        ...
        // migration marker
        const META_RULES = '_beeclear_ilm_rules';

        // per-post internal rules
        const META_ALLOWED_TAGS = '_beeclear_ilm_allowed_tags';

        // per-post tag whitelist for matching
        const META_CONTEXT_FLAG = '_beeclear_ilm_context_flag';

        // per-post flag to enable context words
        const META_NO_OUT = '_beeclear_ilm_no_outgoing';

        // per-post flag: block outgoing autolinks
        const META_MAX_PER_TARGET = '_beeclear_ilm_max_per_target';

        // per-post override: max links per target
        ...
        public function __construct() {
            ...
            add_action(
                'update_option_' . self::OPT_SETTINGS,
                array($this, 'handle_settings_update'),
                10,
                3
            );
            ...
            add_filter(
                'wp_insert_post_data',
                array($this, 'block_empty_phrase_save'),
                10,
                2
            );
            ...
            add_action(
                'save_post',
                array($this, 'trigger_full_rebuild_after_save'),
                99,
                3
            );
            add_action(
                'transition_post_status',
                array($this, 'on_status_change'),
                10,
                3
            );
        ...
        public function activate() {
            ...
            add_option(
                self::OPT_EXTERNAL_MAP,
                array(),
                '',
                false
            );
            add_option(
                self::OPT_OVERVIEW_SCAN_SUMMARY,
                array(),
                '',
                false
            );
            add_option(
                self::OPT_ACTIVITY_LOG,
                array(),
                '',
                false
            );
        ...
        public function admin_menu() {
            ...
            add_submenu_page(
                'beeclear-ilm',
                __( 'Global settings', 'beeclear-smart-link-manager-premium' ),
                __( 'Global settings', 'beeclear-smart-link-manager-premium' ),
                $cap,
                'beeclear-ilm',
                array($this, 'render_dashboard')
            ...
            add_submenu_page(
                'beeclear-ilm',
                __( 'External linking', 'beeclear-smart-link-manager-premium' ),
                __( 'External linking', 'beeclear-smart-link-manager-premium' ),
                $cap,
                'beeclear-ilm-external',
                array($this, 'render_external')
            );
            add_submenu_page(
                'beeclear-ilm',
                __( 'Linking Overview', 'beeclear-smart-link-manager-premium' ),
                __( 'Linking Overview', 'beeclear-smart-link-manager-premium' ),
                $cap,
                'beeclear-ilm-internal-overview',
                array($this, 'render_internal_overview')
            );
            add_submenu_page(
                'beeclear-ilm',
                __( 'Import/Export', 'beeclear-smart-link-manager-premium' ),
                __( 'Import/Export', 'beeclear-smart-link-manager-premium' ),
                $cap,
                'beeclear-ilm-impex',
                array($this, 'render_impex')
            );
        }
        ...
        private function store_link_context(
            &$contexts,
            $target,
            $phrase,
            $node,
...
JS;
        ...
        private function render_author_note() {
            if ( $this->author_note === null ) {
                $this->author_note = '<p class="beeclear-author-note">' . '<span class="dashicons dashicons-admin-users" aria-hidden="true"></span>' . '<span>' . esc_html__( 'Author:', 'beeclear-smart-link-manager-premium' ) . ' <a href="https://beeclear.pl" target="_blank" rel="noopener">BeeClear</a></span>' . '</p>';
            }
        ...
        public function add_metabox() {
            ...
            foreach ( $pts as $pt ) {
                add_meta_box(
                    'beeclear_ilm_box',
                    __( 'BeeClear Smart Link Manager - Phrases and rules', 'beeclear-smart-link-manager-premium' ),
        ...
        public function render_metabox( $post ) {
            ...
            echo '<hr><p><strong>' . esc_html__( 'Admin menu name:', 'beeclear-smart-link-manager-premium' ) . '</strong> ' . '<a href="' . esc_url( $settings_url ) . '" target="_blank" rel="noopener noreferrer">' . esc_html( $menu_name ) . '</a></p>';
        ...
        private function maybe_block_empty_phrase_save( $post_id ) {
            ...
            foreach ( $raw_rules as $rule ) {
                ...
                if ( $phrase === '' ) {
                    wp_die( esc_html__( 'Pole fraza nie może być puste.', 'beeclear-smart-link-manager-premium' ), esc_html__( 'Nie można zapisać.', 'beeclear-smart-link-manager-premium' ), array(
                        'back_link' => true,
                    ) );
                }
            }
        ...
        public function run_async_rebuild_index() {
            ...
            if ( $processed >= count( $ids ) ) {
                ...
                delete_option( self::OPT_REBUILD_STATE );
                $this->log_activity( __( 'Index rebuild completed.', 'beeclear-smart-link-manager-premium' ) );
                return;
            }
        ...
        public function admin_bar_scan_progress( $wp_admin_bar ) {
            ...
            if ( ! empty( $rb_state ) && isset( $rb_state['total'] ) ) {
                ...
                $rb_label = esc_html( sprintf( __( 'Index %d%%', 'beeclear-smart-link-manager-premium' ), $rb_percent ) );
        ...
        public function autolink_content( $content, $force = false ) {
            ...
            try {
                ...
                if ( $external_cache === null ) {
                    $external_cache = get_option( self::OPT_EXT_RULES, array() );
                }
                ...
                if ( class_exists( 'DOMDocument' ) && class_exists( 'DOMXPath' ) ) {
                    ...
                    foreach ( (array) $external as $idx => $r ) {
                        ...
                        if ( !empty( $types ) && $current_post_type && !in_array( $current_post_type, $types, true ) ) {
                            continue;
                        }
                    ...
                    $linked_phrases_internal = array();
                    // per target => [matched_text => count]
                    $linked_contexts_internal = array();
                    // per target => list of fragments (html + tag + phrase)
                    $linked_phrases_external = array();
                    // per rule idx => [matched_text => count]
                    $linked_contexts_external = array();
                    // per rule idx => list of fragments (html + tag + phrase)
                    $total_count = 0;
                    ...
                    foreach ( $textNodes as $node ) {
                        ...
                        if ( !$skip_internal_here ) {
                            foreach ( $prepared_internal as $rule ) {
                                ...
                                if ( !empty( $rule['context_patterns'] ) ) {
                                    ...
                                    if ( !$this->context_patterns_match( $element_text, $rule['context_patterns'] ) ) {
                                        continue;
                                    }
                                }
                                ...
                                if ( $pattern ) {
                                    ...
                                    $newTxt = preg_replace_callback(
                                        ...
                                        function ( $m ) use(
                                            ...
                                            $title = $this->build_attr_from_mode_rule_or_global(
                                                $settings,
                                                null,
                                                'title_mode',
                                                'title_custom',
                                            ...
                                            $aria = $this->build_attr_from_mode_rule_or_global(
                                                $settings,
                                                null,
                                                'aria_mode',
                                                'aria_custom',
                                ...
                                if ( $enable_cross && empty( $rule['regex'] ) && !$this->contains_tokens( $rule['phrase'] ) ) {
                                    if ( $this->cross_inline_try(
                                        $dom,
                                        $node,
                                        $rule['phrase'],
                                        !empty( $rule['case'] ),
                                        function ( $anchor, $matched ) use(
                                            $settings,
                                            $rule,
                                            $target,
                                            &$linked_phrases_internal
                        ...
                        foreach ( $prepared_external as $er ) {
                            $skip_external_here = ( !empty( $er['allowed_tags'] ) ? !$this->node_in_tags( $node, $er['allowed_tags'] ) : $base_skip_external_here );
                            ...
                            if ( $enable_cross && empty( $er['regex'] ) && !$this->contains_tokens( $er['phrase'] ) ) {
                                if ( $this->cross_inline_try(
                                    ...
                                    $er['phrase'],
                                    !empty( $er['case'] ),
                                    function ( $anchor, $matched ) use($settings, $er) {
                                        $url = $er['url'];
                                        if ( !$url ) {
                                            return false;
                                        }
                ...
                return $this->fallback_link_once_both(
                    $content,
                    $internal,
                    $external,
                    $settings,
        ...
        // CROSS-INLINE helper
        private function cross_inline_try(
            DOMDocument $dom,
            DOMText $node,
            $phrase,
            $case,
            callable $configure_anchor,
            ?callable $after_match = null
        ) {
            ...
            $haystack = $t1 . $t2;
            $pos = ( $case ? ( function_exists( 'mb_strpos' ) ? mb_strpos(
                $haystack,
                $phrase,
                0,
                'UTF-8'
            ) : strpos( $haystack, $phrase ) ) : (( function_exists( 'mb_stripos' ) ? mb_stripos(
                $haystack,
                $phrase,
                0,
                'UTF-8'
            ) : stripos( $haystack, $phrase ) )) );
            if ( $pos === false ) {
                return false;
            }
            if ( $pos >= $len1 ) {
            ...
            $need2 = $end - $len1;
            $take1 = $len1 - $pos;
            $before1 = ( function_exists( 'mb_substr' ) ? mb_substr(
                $t1,
                0,
                $pos,
                'UTF-8'
            ) : substr( $t1, 0, $pos ) );
            $match1 = ( function_exists( 'mb_substr' ) ? mb_substr(
                $t1,
                $pos,
                $take1,
                'UTF-8'
            ...
            $match2 = ( function_exists( 'mb_substr' ) ? mb_substr(
                $t2,
                0,
                $need2,
                'UTF-8'
            ) : substr( $t2, 0, $need2 ) );
            $after2 = ( function_exists( 'mb_substr' ) ? mb_substr(
                $t2,
                $need2,
                null,
                'UTF-8'
        ...
        private function build_pattern( $phrase, $is_regex, $is_case ) {
            ...
            if ( $is_regex ) {
                ...
                $q = preg_quote( $phrase, '/' );
                $result = ( $is_case ? '/(?<!' . $boundary . ')(' . $q . ')(?!' . $boundary . ')/u' : '/(?<!' . $boundary . ')(' . $q . ')(?!' . $boundary . ')/iu' );
                $cache[ $cache_key ] = $result;
                return $result;
            }
        }

        private function build_attr_from_mode_rule_or_global(
            $settings,
            $ruleOrNull,
            $mode_key,
            $custom_key,
        ...
        private function fallback_link_once_both(
            $content,
            $internal,
            $external,
            $settings,
            ...
            foreach ( (array) $rules as $idx => $rule ) {
                ...
                foreach ( $sources as $sid => $info ) {
                    ...
                    if ( !isset( $rows[$url]['sources'][$source_id] ) ) {
                        $rows[$url]['sources'][$source_id] = array(
                            'count'    => 0,
                            'phrases'  => array(),
                            'contexts' => array(),
                        );
        ...
        private function render_overview_table() {
            ...
            $base_args = array(
                'page'     => 'beeclear-ilm-internal-overview',
                'ilm_view' => $view,
                '_wpnonce' => $nonce_param,
            );
            if ( $search !== '' ) {
                $base_args['ilm_q'] = $search;
            }
            $base_url = add_query_arg( $base_args, admin_url( 'admin.php' ) );
            $pagination_base = add_query_arg( array_merge( $base_args, array(
                'ilm_page' => '%#%',
            ) ), admin_url( 'admin.php' ) );
            ...
            $toggle_target_url = wp_nonce_url( add_query_arg( array_merge( $base_args, array(
                'ilm_view' => 'targets',
                'ilm_page' => 1,
            ) ), admin_url( 'admin.php' ) ), self::NONCE );
            $toggle_source_url = wp_nonce_url( add_query_arg( array_merge( $base_args, array(
                'ilm_view' => 'sources',
                'ilm_page' => 1,
            ) ), admin_url( 'admin.php' ) ), self::NONCE );
            $toggle_external_url = wp_nonce_url( add_query_arg( array_merge( $base_args, array(
                'ilm_view' => 'external',
                'ilm_page' => 1,
            ) ), admin_url( 'admin.php' ) ), self::NONCE );
            ...
            $popup_script = 'jQuery(function($){'
                ...
                . 'function openPopup(id,view,title,page){currentEntry=id;currentView=view;$title.text(title);$body.html("<p>' . esc_js( __( 'Loading…', 'beeclear-smart-link-manager-premium' ) ) . '</p>");$pagination.empty();$overlay.removeAttr("hidden");loadPopupPage(page||1);}'
                . 'function closePopup(){$overlay.attr("hidden","hidden");$body.empty();$pagination.empty();currentEntry=null;currentView=null;}'
                . 'function loadPopupPage(page){$.post(ajaxurl,{action:"beeclear_ilm_expand_sources_paginated",id:currentEntry,view:currentView,popup_page:page,_ajax_nonce:popupNonce},function(resp){if(!resp||!resp.success){$body.html("<p>' . esc_js( __( 'Error loading data.', 'beeclear-smart-link-manager-premium' ) ) . '</p>");return;}$body.html(resp.data.html);renderPagination(resp.data.page,resp.data.total_pages,resp.data.total_items);}).fail(function(){$body.html("<p>' . esc_js( __( 'Request failed.', 'beeclear-smart-link-manager-premium' ) ) . '</p>");});}'
                . 'function renderPagination(page,totalPages,totalItems){$pagination.empty();if(totalPages<=1){$pagination.append("<span class=\\"beeclear-popup-page-info\\">"+totalItems+" ' . esc_js( __( 'items', 'beeclear-smart-link-manager-premium' ) ) . '</span>");return;}'
                ...
                . 'for(var i=startP;i<=endP;i++){var $btn=$("<button>",{text:i,"data-page":i});if(i===page)$btn.addClass("current");$pagination.append($btn);}'
                ...
                . '$table.on("click","th.beeclear-sort",function(){var $th=$(this),col=parseInt($th.data("col"),10),type=$th.data("sort"),asc=!$th.hasClass("asc");$table.find("th.beeclear-sort").removeClass("asc desc");$th.addClass(asc?"asc":"desc");var $tbody=$table.find("tbody"),$rows=$tbody.find("tr").get();$rows.sort(function(a,b){var $ca=$(a).find("td").eq(col),$cb=$(b).find("td").eq(col);var va=$ca.attr("data-sort-value"),vb=$cb.attr("data-sort-value");if(typeof va==="undefined")va=$ca.text().trim().toLowerCase();if(typeof vb==="undefined")vb=$cb.text().trim().toLowerCase();if(type==="num"){va=parseFloat(va)||0;vb=parseFloat(vb)||0;return asc?va-vb:vb-va;}return asc?va.localeCompare(vb):vb.localeCompare(va);});$.each($rows,function(i,row){$tbody.append(row);});});'
                . '});';
            $h = 'beeclear-ilm-admin-runtime';
            if ( ! wp_script_is( $h, 'registered' ) ) {
                wp_register_script( $h, false, array( 'jquery' ), self::VERSION, true );
            }
            wp_enqueue_script( $h );
        ...
        public function ajax_expand_sources() {
            ...
            if ( $view === 'sources' ) {
                ...
                foreach ( (array) $map as $target_id => $entry ) {
                    ...
                    $info = $sources[$id];
                    $targets[$target_id] = ( is_array( $info ) ? $info : array(
                        'count'    => (int) $info,
                        'phrases'  => array(),
                        'contexts' => array(),
                    ) );
                        ...
                        foreach ( $phrases as $ph => $pc ) {
                            ...
                            if ( $popup_inner !== '' ) {
                                ...
                                if ( $has_manual ) {
                                    ...
                                    $part .= ' <span class="beeclear-ilm-manual-badge" title="' . $manual_label . '" aria-label="' . $manual_label . '">' . '<span class="dashicons dashicons-admin-tools" aria-hidden="true"></span>' . '<span class="screen-reader-text">' . esc_html__( 'Manual insertion link', 'beeclear-smart-link-manager-premium' ) . '</span>' . '</span>';
                                }
                                if ( $popup_id !== '' ) {
                ...
                if ( $target === null ) {
                    echo '<em>' . esc_html__( 'Target not found.', 'beeclear-smart-link-manager-premium' ) . '</em>';
                    wp_die();
                }
        ...
        public function ajax_start_overview_scan() {
            ...
            if ( empty( $ids ) ) {
                delete_option( self::OPT_OVERVIEW_SCAN );
                wp_send_json_error( array(
                    'message' => __( 'No public content found to scan.', 'beeclear-smart-link-manager-premium' ),
                ) );
            }
            update_option( self::OPT_LINKMAP, array(), false );
            update_option( self::OPT_EXTERNAL_MAP, array(), false );
        ...
        public function render_external() {
            ...
            echo wp_kses_post( $this->render_token_tips_html() );
            echo '</div>';
            echo '<form method="post"><table class="widefat striped" id="beeclear-ilm-ext-table"><thead><tr>' . '<th>' . esc_html__( 'Destination & Phrase / Flags', 'beeclear-smart-link-manager-premium' ) . '</th>' . '<th>' . esc_html__( 'Attributes', 'beeclear-smart-link-manager-premium' ) . '</th>' . '<th>' . esc_html__( 'Post types / Exclusions', 'beeclear-smart-link-manager-premium' ) . '</th>' . '<th>' . esc_html__( 'Actions', 'beeclear-smart-link-manager-premium' ) . '</th>' . '</tr></thead><tbody>';
            $i = 0;
            foreach ( $rules as $r ) {
                $phrase = ( isset( $r['phrase'] ) ? $r['phrase'] : '' );
                $regex = !empty( $r['regex'] );
                ...
                echo '<tr>';
                echo '<td class="cell-phrase">' . '<div class="ext-field ext-destination">' . '<label class="ext-destination-label" for="' . esc_attr( $url_id ) . '">' . esc_html__( 'Destination URL', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="url" id="' . esc_attr( $url_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][url]" class="regular-text" value="' . esc_attr( $url ) . '" placeholder="https://example.com">' . '</div>' . '<div class="ext-field ext-phrase">' . '<label class="ext-phrase-label" for="' . esc_attr( $phrase_id ) . '">' . esc_html__( 'Phrase or regex', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="text" id="' . esc_attr( $phrase_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][phrase]" class="regular-text" value="' . esc_attr( $phrase ) . '" placeholder="' . esc_attr__( 'Phrase or regex', 'beeclear-smart-link-manager-premium' ) . '">' . '</div>' . '<div class="flags">' . '<label><input type="checkbox" class="ext-regex" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][regex]" value="1" ' . checked( $regex, true, false ) . '> ' . esc_html__( 'Regex', 'beeclear-smart-link-manager-premium' ) . '</label> ' . '<label><input type="checkbox" class="ext-case" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][case]" value="1" ' . checked( $case, true, false ) . (( $regex ? ' disabled' : '' )) . '> ' . esc_html__( 'Case-sensitive', 'beeclear-smart-link-manager-premium' ) . '</label>' . '</div>' . '<div class="ext-field ext-context">' . '<label class="ext-context-label" for="' . esc_attr( $context_id ) . '">' . esc_html__( 'Context words or regex', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="text" id="' . esc_attr( $context_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][context]" class="regular-text" value="' . esc_attr( $context_display ) . '" placeholder="' . esc_attr__( 'Additional words required in the same element', 'beeclear-smart-link-manager-premium' ) . '">' . '<div class="flags">' . '<label><input type="checkbox" class="ext-context-regex" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][context_regex]" value="1" ' . checked( $context_regex, true, false ) . '> ' . esc_html__( 'Regex', 'beeclear-smart-link-manager-premium' ) . '</label> ' . '<label><input type="checkbox" class="ext-context-case" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][context_case]" value="1" ' . checked( $context_case, true, false ) . (( $context_regex ? ' disabled' : '' )) . '> ' . esc_html__( 'Case-sensitive', 'beeclear-smart-link-manager-premium' ) . '</label>' . '</div>' . '<p class="description">' . esc_html__( 'Supports token syntax (non-regex). Separate multiple entries with commas.', 'beeclear-smart-link-manager-premium' ) . '</p>' . '</div>' . '</td>';
                echo '<td class="cell-attrs"><div class="attr-rows">' . '<div class="ar"><label class="ar-label" for="' . esc_attr( $rel_id ) . '">rel</label><div class="ar-field"><input type="text" id="' . esc_attr( $rel_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][rel]" class="regular-text" value="' . esc_attr( $rel ) . '" placeholder="nofollow noopener"></div></div>' . '<div class="ar"><label class="ar-label" for="' . esc_attr( $title_mode_id ) . '">' . esc_html__( 'Title', 'beeclear-smart-link-manager-premium' ) . '</label><div class="ar-field"><div class="inline-field"><select id="' . esc_attr( $title_mode_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][title_mode]">' . wp_kses_post( $this->options_html( array('none', 'phrase', 'custom'), $title_mode ) ) . '</select></div>' . '<div class="inline-field"><label class="screen-reader-text" for="' . esc_attr( $title_custom_id ) . '">' . esc_html__( 'Custom title', 'beeclear-smart-link-manager-premium' ) . '</label><input type="text" id="' . esc_attr( $title_custom_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][title_custom]" class="regular-text" value="' . esc_attr( $title_custom ) . '" placeholder="' . esc_attr__( 'Custom title', 'beeclear-smart-link-manager-premium' ) . '"></div></div></div>' . '<div class="ar"><label class="ar-label" for="' . esc_attr( $aria_mode_id ) . '">' . esc_html__( 'Aria-label', 'beeclear-smart-link-manager-premium' ) . '</label><div class="ar-field"><div class="inline-field"><select id="' . esc_attr( $aria_mode_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][aria_mode]">' . wp_kses_post( $this->options_html( array('none', 'phrase', 'custom'), $aria_mode ) ) . '</select></div>' . '<div class="inline-field"><label class="screen-reader-text" for="' . esc_attr( $aria_custom_id ) . '">' . esc_html__( 'Custom aria-label', 'beeclear-smart-link-manager-premium' ) . '</label><input type="text" id="' . esc_attr( $aria_custom_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][aria_custom]" class="regular-text" value="' . esc_attr( $aria_custom ) . '" placeholder="' . esc_attr__( 'Custom aria-label', 'beeclear-smart-link-manager-premium' ) . '"></div></div></div>' . '<div class="ar"><label class="ar-label" for="' . esc_attr( $class_id ) . '">CSS class</label><div class="ar-field"><input type="text" id="' . esc_attr( $class_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][class]" class="regular-text" value="' . esc_attr( $class ) . '" placeholder="beeclear-ilm-link"></div></div>' . '</div></td>';
                echo '<td class="cell-types">' . '<div class="types-stack">' . '<div class="max-per-page-field"><label class="max-label" for="' . esc_attr( $max_id ) . '">' . esc_html__( 'Max/page', 'beeclear-smart-link-manager-premium' ) . '</label><input type="number" min="0" id="' . esc_attr( $max_id ) . '" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][max_per_page]" value="' . esc_attr( (int) $max_per_page ) . '"> <span class="desc">' . esc_html__( '0 = unlimited', 'beeclear-smart-link-manager-premium' ) . '</span></div>' . '<div class="types-checklist">' . wp_kses_post( $this->post_types_checklist_html( 'beeclear_ilm_ext[' . intval( $i_attr ) . '][types][]', $types, $pts ) ) . '</div>' . '<div class="field-stack">' . '<label class="field-label" for="beeclear-ilm-ext-' . intval( $i_attr ) . '-exclude">' . esc_html__( 'Exclude by post ID', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="text" id="beeclear-ilm-ext-' . intval( $i_attr ) . '-exclude" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][exclude_ids]" class="regular-text" value="' . esc_attr( $exclude_ids ) . '" placeholder="e.g. 123, 456">' . '</div>' . '<div class="field-stack">' . '<label class="field-label" for="beeclear-ilm-ext-' . intval( $i_attr ) . '-allowed">' . esc_html__( 'Allowed elements (overrides global skip)', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="text" id="beeclear-ilm-ext-' . intval( $i_attr ) . '-allowed" name="beeclear_ilm_ext[' . intval( $i_attr ) . '][allowed_tags]" class="regular-text" value="' . esc_attr( $allowed_tags_raw ) . '" placeholder="p, ul, ol">' . '<p class="description">' . esc_html__( 'Comma-separated tag names. Leave empty to follow global “Skip elements (EXTERNAL)” setting.', 'beeclear-smart-link-manager-premium' ) . '</p>' . '</div>' . '</div>' . '</td>';
            ...
            echo '</ol>';
            echo '</div>';
            echo '<script type="text/html" id="beeclear-ilm-ext-types-template">' . '<div class="types-stack">' . '<div class="max-per-page-field"><label class="max-label" for="beeclear-ilm-ext-__IDX__-max">' . esc_html__( 'Max/page', 'beeclear-smart-link-manager-premium' ) . '</label><input type="number" min="0" id="beeclear-ilm-ext-__IDX__-max" name="beeclear_ilm_ext[__IDX__][max_per_page]" value="1"> <span class="desc">' . esc_html__( '0 = unlimited', 'beeclear-smart-link-manager-premium' ) . '</span></div>' . '<div class="types-checklist">' . wp_kses_post( $this->post_types_checklist_html(
                'beeclear_ilm_ext[__IDX__][types][]',
                array(),
                $pts,
                true
            ) ) . '</div>' . '<div class="field-stack">' . '<label class="field-label" for="beeclear-ilm-ext-__IDX__-exclude">' . esc_html__( 'Exclude by post ID', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="text" id="beeclear-ilm-ext-__IDX__-exclude" name="beeclear_ilm_ext[__IDX__][exclude_ids]" class="regular-text" placeholder="123, 456">' . '</div>' . '<div class="field-stack">' . '<label class="field-label" for="beeclear-ilm-ext-__IDX__-allowed">' . esc_html__( 'Allowed elements (overrides global skip)', 'beeclear-smart-link-manager-premium' ) . '</label>' . '<input type="text" id="beeclear-ilm-ext-__IDX__-allowed" name="beeclear_ilm_ext[__IDX__][allowed_tags]" class="regular-text" placeholder="p, ul, ol">' . '<p class="description">' . esc_html__( 'Comma-separated tag names. Leave empty to follow global “Skip elements (EXTERNAL)” setting.', 'beeclear-smart-link-manager-premium' ) . '</p>' . '</div>' . '</div>' . '</script>';
            if ( !empty( $rules ) ) {
                echo '<div class="beeclear-card">';
        ...
        private function post_types_checklist_html(
            $name,
            $selected,
            $pts,
            $template = false
        ...
        <div class="wrap">
            <div class="ilm-wrap">
                <h1><?php 
            esc_html_e( 'Import/Export', 'beeclear-smart-link-manager-premium' );
            ?></h1>
                <?php 
            echo wp_kses_post( $this->render_author_note() );
            ?>
                <div class="beeclear-card">
                    <h2><?php 
            esc_html_e( 'Export', 'beeclear-smart-link-manager-premium' );
            ?></h2>
                    <textarea rows="16" class="large-text code" readonly id="beeclear-ilm-export-json"><?php 
            echo esc_textarea( $json );
            ?></textarea>
                    <p><button type="button" class="button" id="beeclear-ilm-export-download"><?php 
            esc_html_e( 'Download JSON file', 'beeclear-smart-link-manager-premium' );
            ?></button></p>
                </div>

                <div class="beeclear-card">
                    <h2><?php 
            esc_html_e( 'Import', 'beeclear-smart-link-manager-premium' );
            ?></h2>
                    <form method="post">
                        <?php 
            wp_nonce_field( self::NONCE, self::NONCE );
            ?>
                        ...
                        <textarea name="beeclear_ilm_json" rows="10" class="large-text code" id="beeclear-ilm-import-json" placeholder="<?php 
            esc_attr_e( 'Paste JSON here…', 'beeclear-smart-link-manager-premium' );
            ?>"></textarea>
                        <p><button class="button button-primary" name="beeclear_ilm_import" value="1"><?php 
            esc_html_e( 'Import', 'beeclear-smart-link-manager-premium' );
            ?></button></p>
                    </form>
                </div>
