...
if (!class_exists('BeeClear_ILM', false)):
    ...
    class BeeClear_ILM
    {
        ...
        /**
         * Tracks when the promotional admin notice was last dismissed per user.
         */
        const META_PROMO_DISMISSED_AT = 'beeclear_ilm_promo_dismissed_at';
        const META_PROMO_DISMISSED = 'beeclear_ilm_promo_dismissed';
        ...
        const OPT_REBUILD_CHUNK_PREFIX = 'beeclear_ilm_rebuild_chunk_'; // temporary async rebuild chunks
        ...
        public function __construct()
        {
            ...
            add_action('update_option_' . self::OPT_SETTINGS, array($this, 'handle_settings_update'), 10, 2);
        ...
        public static function uninstall()
        {
            ...
            if (!empty($settings['clean_on_uninstall'])) {
                ...
                delete_option(self::OPT_PROMO_START);
        ...
        public function maybe_upgrade()
        {
            ...
            if (!current_user_can('manage_options'))
                return;
            if (apply_filters('beeclear_ilm_skip_migration', defined('BEECLEAR_ILM_SKIP_MIGRATION') && BEECLEAR_ILM_SKIP_MIGRATION))
                return;

            $dbver = get_option(self::OPT_DBVER, '');
            if (version_compare($dbver ?: '0.0.0', self::VERSION, '>='))
                return;
            ...
            if (version_compare($dbver ?: '0.0.0', '1.4.0', '<')) {
                ...
                $settings = get_option(self::OPT_SETTINGS, array());
                if (isset($settings['case_sensitive']))
                    unset($settings['case_sensitive']);
                if (isset($settings['allow_regex']))
                    unset($settings['allow_regex']);
                if (!isset($settings['skip_elements_internal']))
                    $settings['skip_elements_internal'] = '';
                if (!isset($settings['skip_elements_external']))
                    $settings['skip_elements_external'] = '';
                ...
                try {
                    ...
                    do {
                        ...
                        foreach ($ids as $pid) {
                            ...
                            foreach ($rules as &$r) {
                                if (!is_array($r)) {
                                    $r = array('phrase' => (string) $r);
                                    $changed = true;
                                }
                                if (!array_key_exists('regex', $r)) {
                                    $r['regex'] = !empty($r['regex']);
                                    $changed = true;
                                }
                                if (!array_key_exists('case', $r)) {
                                    $r['case'] = false;
                                    $changed = true;
                                }
                            }
        ...
        public function settings_link($links)
        {
            ...
            $links[] = '<a href="' . esc_url($url) . '">' . esc_html__('Settings', 'beeclear-smart-link-manager') . '</a>';
            return $links;
        }
        ...
        {
            $cap = 'manage_options';
            $title = __('BeeClear Smart Link Manager', 'beeclear-smart-link-manager');
            add_menu_page($title, $title, $cap, 'beeclear-ilm', array($this, 'render_dashboard'), 'dashicons-admin-links', 59);
            add_submenu_page('beeclear-ilm', __('Global settings', 'beeclear-smart-link-manager'), __('Global settings', 'beeclear-smart-link-manager'), $cap, 'beeclear-ilm', array($this, 'render_dashboard'));
            add_submenu_page('beeclear-ilm', __('Linking Overview', 'beeclear-smart-link-manager'), __('Linking Overview', 'beeclear-smart-link-manager'), $cap, 'beeclear-ilm-internal-overview', array($this, 'render_internal_overview'));
            add_submenu_page('beeclear-ilm', __('Import/Export', 'beeclear-smart-link-manager'), __('Import/Export', 'beeclear-smart-link-manager'), $cap, 'beeclear-ilm-impex', array($this, 'render_impex'));
        }
        ...
        public function sanitize_settings($in)
{
    ...
    $allowed_modes = array('none', 'phrase', 'post_title', 'custom');

    $title_mode = isset($in['title_mode']) ? $in['title_mode'] : 'phrase';
    $out['title_mode'] = in_array($title_mode, $allowed_modes, true) ? $title_mode : 'phrase';

    $out['title_custom'] = isset($in['title_custom']) ? sanitize_text_field($this->normalize_text_setting($in['title_custom'])) : '';

    $aria_mode = isset($in['aria_mode']) ? $in['aria_mode'] : 'phrase';
    $out['aria_mode'] = in_array($aria_mode, $allowed_modes, true) ? $aria_mode : 'phrase';
    ...
    // Numeric fields
    $out['max_per_target']     = max(0, intval(isset($in['max_per_target']) ? $in['max_per_target'] : 1));
    $out['max_total_per_page'] = max(0, intval(isset($in['max_total_per_page']) ? $in['max_total_per_page'] : 0));
    $out['min_content_length'] = max(0, intval(isset($in['min_content_length']) ? $in['min_content_length'] : 200));
    $out['min_element_length'] = max(0, intval(isset($in['min_element_length']) ? $in['min_element_length'] : 20));

    // Post types
    $post_types = isset($in['process_post_types']) ? (array) $in['process_post_types'] : array('post', 'page');
    $post_types = array_map('sanitize_key', $post_types);
    $post_types = array_filter($post_types);
    $out['process_post_types'] = array_values($post_types);

    // Safe link_template: enforce {url} and {text}, allow only <a> with safe attributes.
    ...
    if (false === strpos($tpl, '{url}') || false === strpos($tpl, '{text}')) {
        $tpl = $default_tpl;
    }
    ...
    return $out;
        ...
        private function sanitize_external_rules($raw_rules)
        {
            ...
            foreach ((array) $raw_rules as $r) {
                ...
                $exclude_ids = array_values(array_unique(array_filter(array_map('intval', preg_split('/[\s,;]+/', $exclude_ids_raw)), function ($id) {
                    return $id > 0;
                })));
        ...
        private function sanitize_imported_post_rules($rules)
        {
            if (!is_array($rules)) {
                return array();
            }
        ...
        private function export_post_payload($post_id)
        {
            ...
            $no_out = !empty(get_post_meta($post_id, self::META_NO_OUT, true));
            $context_enabled = !empty(get_post_meta($post_id, self::META_CONTEXT_FLAG, true));

            if (!$rules && $limit === '' && $allowed_tags === '' && !$no_out && !$context_enabled) {
                return null;
            }
        ...
        private function import_post_payload($post_id, $payload)
        {
            ...
            if (is_array($rules)) {
                update_post_meta($post_id, self::META_RULES, $this->sanitize_imported_post_rules($rules));
            }
            ...
            $no_outgoing = is_array($payload) && !empty($payload['no_outgoing']);
            if ($no_outgoing) {
        ...
        private function process_overview_scan_batch($batch_size = 5)
        {
            ...
            if ($done) {
                ...
                $this->log_activity(sprintf(
                    ...
                    __('Scan finished: %1$d pages, %2$d internal links, %3$d external links.', 'beeclear-smart-link-manager'),
                    (int) $summary['scanned'],
        ...
        public function maybe_handle_promo_dismissal()
        {
            // Deprecated: Dismissal is now handled via AJAX only
        }
        ...
        public function ajax_dismiss_promo()
        {
            ...
            check_ajax_referer('beeclear_ilm_dismiss_promo', 'nonce');

            $user_id = get_current_user_id();
            if (!$user_id) {
                wp_send_json_error('No user ID', 400);
            }

            // Save dismissal timestamp
            $timestamp = time();
            update_user_meta($user_id, self::META_PROMO_DISMISSED_AT, $timestamp);

            // Clean up old flag if exists
            delete_user_meta($user_id, self::META_PROMO_DISMISSED);

            wp_send_json_success(array('timestamp' => $timestamp));
        }

        public function render_promo_notice()
{
    ...
    // Show only on this plugin's admin screens.
    $allowed_screens = array(
        'toplevel_page_beeclear-ilm',
        'beeclear-ilm_page_beeclear-ilm',
        'beeclear-ilm_page_beeclear-ilm-internal-overview',
        'beeclear-ilm_page_beeclear-ilm-impex',
    );
    ...
    // If dismissed once, do not show again.
    $dismissed_at = (int) get_user_meta($user_id, self::META_PROMO_DISMISSED_AT, true);
    if ($dismissed_at > 0) {
        return;
    }

    $nonce = wp_create_nonce('beeclear_ilm_dismiss_promo');
    ?>
    <div class="notice notice-info is-dismissible beeclear-ilm-promo-notice" data-nonce="<?php echo esc_attr($nonce); ?>">
        <p><strong><?php esc_html_e('Check the premium version of the BeeClear Smart Link Manager plugin and:', 'beeclear-smart-link-manager'); ?></strong></p>
        <ul>
            <li><strong><?php esc_html_e('Enhance semantics', 'beeclear-smart-link-manager'); ?></strong> <?php esc_html_e('by defining context for each link.', 'beeclear-smart-link-manager'); ?></li>
            <li><?php esc_html_e('Create your own', 'beeclear-smart-link-manager'); ?> <strong><?php esc_html_e('external linking', 'beeclear-smart-link-manager'); ?></strong> <?php esc_html_e('rules.', 'beeclear-smart-link-manager'); ?></li>
            <li><?php esc_html_e('Use additional', 'beeclear-smart-link-manager'); ?> <strong><?php esc_html_e('linking rules.', 'beeclear-smart-link-manager'); ?></strong></li>
    ...
    $script = "document.addEventListener('click', function (event) {\n"
        ...
        . "        return;\n"
        . "    }\n"
        . "    var dismissBtn = target.closest('.notice-dismiss');\n"
        . "    if (!dismissBtn) {\n"
        . "        return;\n"
        . "    }\n"
        . "    var notice = dismissBtn.closest('.beeclear-ilm-promo-notice');\n"
        . "    if (!notice) {\n"
        . "        return;\n"
        . "    }\n"
        ...
        . "        action: 'beeclear_ilm_dismiss_promo',\n"
        . "        nonce: nonce\n"
        . "    });\n"
        . "    var body = payload.toString();\n"
        . "    if (navigator.sendBeacon) {\n"
        . "        var beaconPayload = new Blob([body], {\n"
        . "            type: 'application/x-www-form-urlencoded; charset=UTF-8'\n"
        . "        });\n"
        . "        var sent = navigator.sendBeacon(ajaxurl, beaconPayload);\n"
        . "        if (sent) {\n"
        . "            return;\n"
        . "        }\n"
        . "    }\n"
    ...
    wp_register_script('beeclear-ilm-promo', '', array('jquery'), self::VERSION, true);
    wp_enqueue_script('beeclear-ilm-promo');
    wp_add_inline_script('beeclear-ilm-promo', $script, 'after');
}
...
public function admin_assets($hook)
        {
            ...
            $L = array(
                'phrase_or_regex' => __('Phrase or regex', 'beeclear-smart-link-manager'),
                'phrase' => __('Phrase', 'beeclear-smart-link-manager'),
                'regex' => __('Regex', 'beeclear-smart-link-manager'),
                'case_sensitive' => __('Case-sensitive', 'beeclear-smart-link-manager'),
                'remove' => __('Remove', 'beeclear-smart-link-manager'),
                'erase' => __('Erase', 'beeclear-smart-link-manager'),
                'title' => __('Title', 'beeclear-smart-link-manager'),
                'aria_label' => __('Aria-label', 'beeclear-smart-link-manager'),
                'custom_title' => __('Custom title', 'beeclear-smart-link-manager'),
                'custom_aria' => __('Custom aria-label', 'beeclear-smart-link-manager'),
                'max_page' => __('Max/page', 'beeclear-smart-link-manager'),
                'zero_unlimited' => __('0 = unlimited', 'beeclear-smart-link-manager'),
                'context_words' => __('Context words', 'beeclear-smart-link-manager'),
                'context_tokens' => __('Supports token syntax (non-regex). Separate multiple entries with commas.', 'beeclear-smart-link-manager'),
                'context_placeholder' => __('Additional words required in the same element', 'beeclear-smart-link-manager'),
                'copy_shortcode' => __('Copy shortcode', 'beeclear-smart-link-manager'),
                'copied' => __('Copied', 'beeclear-smart-link-manager'),
                'scan_failed' => __('Scan failed.', 'beeclear-smart-link-manager'),
                'scan_done' => __('Scan finished. Overview updated.', 'beeclear-smart-link-manager'),
                'scan_prepare' => __('Preparing scan…', 'beeclear-smart-link-manager'),
...
jQuery(function($){
    ...
    $("#beeclear-ilm-add").on("click", function(e){
        ...
        var row = '<div class="ilm-row"><span class="handle" aria-hidden="true">☰</span>'
            ...
            +       '<input type="text" name="beeclear_ilm_rules['+idx+'][phrase]" class="regular-text" placeholder="'+(L.phrase||L.phrase_or_regex||'Phrase')+'" aria-label="'+(L.phrase||L.phrase_or_regex||'Phrase')+'">'
    ...
    if($logWrap.length){
        ...
        private function admin_css_block()
        {
			...
			.beeclear_ilm.column-beeclear_ilm strong{margin-right: 5px;}
			.beeclear-ilm-promo-notice ul{list-style:disc;padding-left:30px;margin:0px;}
            ...
            .beeclear-ilm-context-popup{position:fixed;z-index:100200;max-width:90%;width:700px;background:#fff;border:1px solid #dcdcde;box-shadow:0 8px 24px rgba(0,0,0,.12);border-radius:8px;padding:12px;}
        ...
        private function render_author_note()
        {
            if ($this->author_note === null) {
                $this->author_note = '<p class="beeclear-author-note">'
                    ...
                    . '<span>' . esc_html__('Author:', 'beeclear-smart-link-manager') . ' <a href="https://beeclear.pl" target="_blank" rel="noopener">BeeClear</a></span>'
                    . '</p>';
            }
        ...
        private function render_token_tips_html()
        {
            if ($this->token_tips_html === null) {
                $this->token_tips_html = '<details class="ilm-tips"><summary>' . esc_html__('Tips: Token syntax (non-regex mode)', 'beeclear-smart-link-manager') . '</summary>';
                $this->token_tips_html .= '<div class="description">';
                $this->token_tips_html .= '<ul style="list-style:disc; padding-left:18px">';
                $this->token_tips_html .= '<li><code>wordpre[string]</code>, <code>[string:5]</code>, <code>[string:max5]</code>, <code>[string:min3]</code> — ' . esc_html__('letters appended to prefix (letters only, no hyphen/space).', 'beeclear-smart-link-manager') . '</li>';
                $this->token_tips_html .= '<li><code>wordpress [words] plugin</code>, <code>[words:1]</code>, <code>[words:max2]</code>, <code>[words:min2]</code> — ' . esc_html__('word(s) between parts, default 1–3.', 'beeclear-smart-link-manager') . '</li>';
                $this->token_tips_html .= '<li>' . esc_html__('Tokens provide flexible phrase matching. You can mix tokens, e.g. ', 'beeclear-smart-link-manager') . '<code>wordpre[string:max5] [words:max2] plug[string:max5]</code></li>';
        ...
        private function render_shortcode_copy_row_html()
        {
            ...
            $out .= '<span class="beeclear-ilm-shortcodes__label">' . esc_html__('Token examples to copy:', 'beeclear-smart-link-manager') . '</span>';
            foreach ($shortcodes as $shortcode) {
                $out .= '<code class="beeclear-ilm-shortcode-chip" data-label="' . esc_attr($shortcode) . '" data-shortcode="' . esc_attr($shortcode) . '" title="' . esc_attr__('Copy shortcode', 'beeclear-smart-link-manager') . '">' . esc_html($shortcode) . '</code>';
            }
            $out .= '</div>';
            return $out;
        }
        ...
        public function add_metabox()
        {
            ...
            $pts = !empty($settings['process_post_types']) ? (array) $settings['process_post_types'] : array('post', 'page');
            foreach ($pts as $pt) {
                add_meta_box('beeclear_ilm_box', __('BeeClear Smart Link Manager - Phrases and rules', 'beeclear-smart-link-manager'), array($this, 'render_metabox'), $pt, 'normal', 'high');
            }
        }
        ...
        public function render_metabox($post)
        {
            ...
            echo '<p>' . esc_html__('Add phrases. Order matters: top has the highest priority.', 'beeclear-smart-link-manager') . '</p>';
            ...
            foreach ($rules as $r) {
                ...
                $case = !empty($r['case']);
                $phrase_label = esc_html__('Phrase', 'beeclear-smart-link-manager');
                $index_attr = (int) $i;
                ...
                printf(
                    ...
                    esc_attr($index_attr),
                    checked($case, true, false),
                    esc_html__('Case-sensitive', 'beeclear-smart-link-manager')
                );
                echo '<a href="#" class="button link-delete">' . esc_html__('Remove', 'beeclear-smart-link-manager') . '</a>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            ...
            echo '</div>';
            echo '<p><a href="#" class="button" id="beeclear-ilm-add">' . esc_html__('Add phrase/rule', 'beeclear-smart-link-manager') . '</a></p>';

            echo '<div class="ilm-metabox-settings-grid">';
            echo '<div class="ilm-metabox-card">';
            echo '<h3 class="ilm-metabox-title">' . esc_html__('Search scope', 'beeclear-smart-link-manager') . '</h3>';
            echo '<div class="ilm-metabox-field">';
            echo '<label class="ilm-metabox-label">' . esc_html__('HTML elements allowed to link to the target', 'beeclear-smart-link-manager') . '</label>';
            echo '<input type="text" name="beeclear_ilm_allowed_elements" value="' . esc_attr($allowed_tags_raw) . '" class="regular-text" placeholder="p, li, h2" aria-describedby="ilm-allowed-elements-help">';
            echo '<p id="ilm-allowed-elements-help" class="description">' . esc_html__('This setting works in parallel with Skip elements (INTERNAL) from the global settings.', 'beeclear-smart-link-manager') . '</p>';
            echo '</div>';
            echo '</div>';

            echo '<div class="ilm-metabox-card">';
            echo '<h3 class="ilm-metabox-title">' . esc_html__('Per-target linking rules', 'beeclear-smart-link-manager') . '</h3>';
            echo '<div class="ilm-metabox-field">';
            echo '<label class="ilm-metabox-label" for="beeclear-ilm-max-per-target">' . esc_html__('Max links per INTERNAL target (this page)', 'beeclear-smart-link-manager') . '</label>';
            echo '<input id="beeclear-ilm-max-per-target" type="number" min="0" name="beeclear_ilm_max_per_target" value="' . esc_attr($max_per_target_override) . '" placeholder="' . esc_attr__('Use global', 'beeclear-smart-link-manager') . '">';
            echo '<p class="description">' . esc_html__('Leave empty to use the global cap. 0 = unlimited for this target.', 'beeclear-smart-link-manager') . '</p>';
            echo '</div>';
            ...
            echo esc_html__('Disable autolinking from this post (no outgoing links)', 'beeclear-smart-link-manager') . '</label>';
            echo '<p class="description">' . esc_html__('When enabled, the plugin will not inject internal or external links into this post content.', 'beeclear-smart-link-manager') . '</p>';
            echo '</div>';
            echo '</div>';
            echo '</div>';

            // echo '<p><label><input type="checkbox" name="beeclear_ilm_rules_clear" value="1"> '.
            //     esc_html__('Clear all phrases on save (explicit action)', 'beeclear-smart-link-manager').
            //     '</label><br><span class="description">'.
            //     esc_html__('If not checked, the plugin will NEVER wipe phrases because of empty/partial POST (e.g., after closing an autosave notice).', 'beeclear-smart-link-manager').
            //     '</span></p>';

            $settings_url = esc_url(admin_url('admin.php?page=beeclear-ilm'));
            $menu_name = esc_html__('BeeClear Smart Link Manager', 'beeclear-smart-link-manager');
            echo '<hr><p><strong>' . esc_html__('Admin menu name:', 'beeclear-smart-link-manager') . '</strong> '
                . '<a href="' . esc_url($settings_url) . '" target="_blank" rel="noopener noreferrer">' . esc_html($menu_name) . '</a></p>';
            echo '<p><strong>' . esc_html__('Author:', 'beeclear-smart-link-manager') . '</strong> <a href="https://beeclear.pl">BeeClear</a></p>';
        }
        ...
        public function save_post_rules($post_id)
        {
            ...
            if ($allowed_clean === '') {
                ...
                if ($current_allowed !== $allowed_clean) {
                    update_post_meta($post_id, self::META_ALLOWED_TAGS, $allowed_clean);
                    $meta_changed = true;
                }
            }
            ...
            $explicit_clear = isset($_POST['beeclear_ilm_rules_clear']) && '1' === sanitize_text_field(wp_unslash($_POST['beeclear_ilm_rules_clear']));
            if ($explicit_clear) {
            ...
            // IMPORTANT: Do not run sanitize_text_field() on the whole rules array.
// It turns arrays into the string "Array" and breaks saving.
            $raw_input = isset($_POST['beeclear_ilm_rules'])
                ? wp_unslash($_POST['beeclear_ilm_rules']) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized per-field below.
                : array();

            // Accept either a proper array (normal form post) or a JSON string (some editors/bots).
            ...
            $rules = array();
            foreach ($raw as $r) {
                $phrase = isset($r['phrase']) ? sanitize_text_field(trim((string) $r['phrase'])) : '';
                if ($phrase === '')
                    continue;
                $case = !empty($r['case']);
                ...
                $rules[] = array(
                    ...
                    // "context" inputs were removed from the post editor UI.
                    // Keep the keys for backward compatibility, but store empty values.
                    'context' => array(),
                    'context_regex' => false,
                    'context_case' => false,
                );
        ...
        public function block_empty_phrase_save($data, $postarr)
        {
            ...
            // IMPORTANT: Do not run sanitize_text_field() on the whole rules array.
            // It turns arrays into the string "Array" and breaks saving.
            $raw_input = array_key_exists('beeclear_ilm_rules', $_POST)
                ? wp_unslash($_POST['beeclear_ilm_rules']) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized per-field below.
                : array();
            ...
            if ($has_empty || (!$has_non_empty && !empty($current_rules))) {
                wp_die(
                    esc_html__("You can't save empty field with phrase", 'beeclear-smart-link-manager'),
                    esc_html__('Error', 'beeclear-smart-link-manager'),
        ...
        {
            $settings = get_option(self::OPT_SETTINGS, array());
            if (empty($settings['auto_scan_on_save']))
                return;

            $pts = !empty($settings['process_post_types']) ? (array) $settings['process_post_types'] : array('post', 'page');
            /* translators: %d: number of pages queued for the overview scan. */
            $this->start_overview_scan_now($pts, __('Auto scan started after save: %d pages queued.', 'beeclear-smart-link-manager'), false);
        }
        ...
        private function rebuild_index()
        {
            ...
            if (!class_exists('WP_Query')) {
                $this->last_rebuild_error = __('Index rebuild failed: WP_Query is unavailable.', 'beeclear-smart-link-manager');
                update_option(self::OPT_INDEX, array(), false);
                return array();
            }
            ...
            if (is_wp_error($q)) {
                ...
                $this->last_rebuild_error = sprintf(__('Index rebuild failed: %s', 'beeclear-smart-link-manager'), $q->get_error_message());
                update_option(self::OPT_INDEX, array(), false);
                return array();
            }
        ...
        public function autolink_excerpt($excerpt)
        {
            return $this->autolink_content($excerpt, true);
        }
        public function autolink_widget_text($text)
        {
            return $this->autolink_content($text, true);
        }
        ...
        public function autolink_content($content, $force = false)
        {
            ...
            try {
                ...
                if ((is_admin() || is_feed() || (defined('REST_REQUEST') && REST_REQUEST) || (defined('DOING_AJAX') && DOING_AJAX)) && !$allow_admin_processing)
                    return $content;
                if (!is_string($content) || $content === '')
                    return $content;
                ...
                static $internal_cache = null;
                if ($internal_cache === null)
                    $internal_cache = get_option(self::OPT_INDEX, array());
                ...
                foreach ((array) $internal as $rule) {
                    ...
                    if ($phrase === '' || !$target)
                        continue;
                    if ($current_id && $target === $current_id)
                        continue;
                    $rule_limit = null;
                ...
                if (class_exists('DOMDocument') && class_exists('DOMXPath')) {
                    ...
                    if (empty($prepared_internal) && empty($prepared_external))
                        return $content;
                    ...
                    if (function_exists('mb_encode_numericentity')) {
                        $wrapped_content = mb_encode_numericentity($content, array(0x80, 0x10FFFF, 0, 0xFFFF), $enc);
                    } else {
                    ...
                    foreach ($textNodes as $node) {
                        ...
                        for ($n = $node->parentNode; $n; $n = $n->parentNode) {
                            ...
                            if ($n->attributes && $n->attributes->getNamedItem('class')) {
                                if (strpos(' ' . $n->attributes->getNamedItem('class')->nodeValue . ' ', ' beeclear-ilm--no-autolink ') !== false) {
                                    $skip_base = true;
                                    break;
                                }
                            }
                    ...
                    foreach ($prepared_internal as $rule) {
                        ...
                        if ($target_limit > 0 && isset($linked_counts_internal[$target]) && $linked_counts_internal[$target] >= $target_limit)
                            continue;
                        ...
                        foreach ($nodes as $node) {
                            ...
                            if (!$meta || $meta['skip_base'] || $meta['skip_short'] || $meta['skip_internal'])
                                continue;

                            if ($max_total > 0 && $total_count >= $max_total) {
                            ...
                            if ($pattern) {
                                ...
                                $newTxt = preg_replace_callback($pattern, function ($m) use ($rule, $target, $settings, &$found, &$linked_phrases_internal, &$matched_phrase) {
                                    ...
                                    $ph = (string) $text;
                                    if ($ph !== '') {
                                        if (!isset($linked_phrases_internal[$target]))
                                            $linked_phrases_internal[$target] = array();
                            ...
                            if ($enable_cross && empty($rule['regex']) && !$this->contains_tokens($rule['phrase'])) {
                                if (
                                    $this->cross_inline_try($dom, $node, $rule['phrase'], !empty($rule['case']), function ($anchor, $matched) use ($settings, $rule, $target, &$linked_phrases_internal) {
                                        $url = $rule['url'];
                                        if (!$url)
                                            return false;
                                        if ($settings['rel'] ?? '')
                                            $anchor->setAttribute('rel', $settings['rel']);
                        ...
                        foreach ($nodes as $node) {
                            ...
                            foreach ($prepared_external as $er) {
                                $skip_external_here = !empty($er['allowed_tags'])
                                    ...
                                    : $meta['base_skip_external'];
                                if ($skip_external_here)
                                    continue;
                            ...
                            if (!empty($er['context_patterns'])) {
                                ...
                                if (!$this->context_patterns_match($element_text, $er['context_patterns']))
                                    continue;
                            }
                                    ...
                                    if ($appended) {
                                        ...
                                        if ($matched_phrase !== null) {
                                            if (!isset($linked_phrases_external[$idx]))
                                                $linked_phrases_external[$idx] = array();
                            ...
                            if ($enable_cross && empty($er['regex']) && !$this->contains_tokens($er['phrase'])) {
                                if (
                                    $this->cross_inline_try($dom, $node, $er['phrase'], !empty($er['case']), function ($anchor, $matched) use ($settings, $er) {
                                        $url = $er['url'];
                                        if (!$url)
                                            return false;
                                        if ($er['rel'] ?? '')
                                            $anchor->setAttribute('rel', $er['rel']);
                    ...
                    if (!empty($post->ID)) {
                        ...
                        foreach ($map as $target_id => $entry) {
                            if (!isset($entry['sources'][$current_id]))
                                continue;
                    ...
                    $html = $dom->saveHTML();
                    if (substr($html, 0, 5) === '<div>' && substr($html, -6) === '</div>') {
        ...
        private function cross_inline_try(DOMDocument $dom, DOMText $node, $phrase, $case, callable $configure_anchor, ?callable $after_match = null)
        {
            ...
            if (!($parent instanceof DOMElement))
                return false;
            if (!$this->is_format_tag($parent->nodeName))
                return false;
            ...
            if ($after2 !== '') {
                ...
                if ($after->parentNode)
                    $after->parentNode->insertBefore($restNode, $after->nextSibling);
            }
            ...
            if (!$configure_anchor($a, $matchedText))
                return false;
            ...
            while (preg_match($re, $phrase, $m, $flags = PREG_OFFSET_CAPTURE, $offset)) {
                ...
                if ($type === 'string') {
                    ...
                    if ($num_exact > 0) {
                        ...
                        $minW = $num_from_minmax;
                        $maxW = -1;
                    } elseif ($minmax === 'max' && $num_from_minmax > 0) {
                        $minW = 1;
                        $maxW = $num_from_minmax;
                    } else {
                        $minW = 1;
                        $maxW = 3;
                    }
        ...
        private function build_attr_from_mode_rule_or_global($settings, $ruleOrNull, $mode_key, $custom_key, $target_id, $phrase_text)
        {
            if ($ruleOrNull && isset($ruleOrNull[$mode_key])) {
                ...
                switch ($mode) {
                    case 'none':
                        return '';
                    case 'custom':
                        return (string) ($ruleOrNull[$custom_key] ?? '');
                    case 'phrase':
                    default:
                        return $phrase_text;
                }
            ...
            switch ($mode) {
                case 'none':
                    return '';
                case 'post_title':
                    return $target_id ? get_the_title($target_id) : '';
                case 'custom':
                    return (string) ($settings[$custom_key] ?? '');
                case 'phrase':
                default:
                    return $phrase_text;
            }
        ...
        private function fallback_link_once_both($content, $internal, $external, $settings, $post)
        {
            ...
            if (is_array($internal)) {
                foreach ($internal as $rule) {
                    $target = (int) $rule['target'];
                    if ($post instanceof WP_Post && $target === (int) $post->ID)
                        continue;
        ...
        public function frontend_assets()
        {
            ...
            if ( is_admin_bar_showing() && current_user_can( 'manage_options' ) ) {
                ...
                if ( $has_scan || $has_rebuild ) {
                    ...
                    $settings_url = esc_url( admin_url( 'admin.php?page=beeclear-ilm' ) );
                    $scan_running_label   = esc_js( __( 'Overview scan in progress…', 'beeclear-smart-link-manager' ) );
                    $rebuild_running_label = esc_js( __( 'Index rebuild…', 'beeclear-smart-link-manager' ) );
...
JS;
        ...
        public function render_timing_log_script()
        {
            ...
            $settings = get_option(self::OPT_SETTINGS, array());
            if (empty($settings['log_internal_timing']))
                return;
            if ($this->autolink_timing_ms <= 0)
                return;

            $ms = round($this->autolink_timing_ms, 2);
            wp_enqueue_script('jquery');
            /* translators: %.2f: time in milliseconds added by internal linking. */
            $message = sprintf(__('Internal linking added %.2f ms to render time.', 'beeclear-smart-link-manager'), $ms);
        ...
        public function render_dashboard()
        {
            ...
            echo '<div class="wrap">';
            echo '<h1>' . esc_html__('BeeClear Smart Link Manager - Global settings', 'beeclear-smart-link-manager') . '</h1>';
            ...
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-admin-generic" aria-hidden="true"></span>' . esc_html__('Global settings', 'beeclear-smart-link-manager') . '</h2>';
            echo '<p class="description">' . esc_html__('Fine-tune how automatic links look, where they appear, and how the plugin maintains its data.', 'beeclear-smart-link-manager') . '</p>';
            echo '</div>';

            echo '<div class="beeclear-card">';
            echo '<h3 class="ilm-section-title"><span class="dashicons dashicons-admin-links" aria-hidden="true"></span>' . esc_html__('Link output defaults', 'beeclear-smart-link-manager') . '</h3>';
            echo '<table class="form-table" role="presentation">';
            echo '<tr><th>' . esc_html__('Default rel attribute', 'beeclear-smart-link-manager') . '</th><td><input type="text" name="' . esc_attr(self::OPT_SETTINGS) . '[rel]" value="' . esc_attr($s['rel']) . '" class="regular-text"><span class="inline-help">' . esc_html__('e.g. nofollow, sponsored, noopener', 'beeclear-smart-link-manager') . '</span></td></tr>';

            echo '<tr><th>' . esc_html__('Default Title & Aria for INTERNAL links', 'beeclear-smart-link-manager') . '</th><td>';
            echo '<div class="ilm-field-grid">';
            echo '<label>' . esc_html__('Title', 'beeclear-smart-link-manager') . '<br><select name="' . esc_attr(self::OPT_SETTINGS) . '[title_mode]">';
            foreach (array('none', 'phrase', 'post_title', 'custom') as $mode) {
                printf('<option value="%1$s"%2$s>%1$s</option>', esc_attr($mode), selected($s['title_mode'], $mode, false));
            }
            echo '</select></label>';
            echo '<label>' . esc_html__('Aria-label', 'beeclear-smart-link-manager') . '<br><select name="' . esc_attr(self::OPT_SETTINGS) . '[aria_mode]">';
            foreach (array('none', 'phrase', 'post_title', 'custom') as $mode) {
                printf('<option value="%1$s"%2$s>%1$s</option>', esc_attr($mode), selected($s['aria_mode'], $mode, false));
            }
            echo '</select></label></div>';

            echo '<div class="ilm-field-grid">';
            echo '<input type="text" name="' . esc_attr(self::OPT_SETTINGS) . '[title_custom]" value="' . esc_attr($s['title_custom']) . '" placeholder="' . esc_attr__('Custom title text', 'beeclear-smart-link-manager') . '" class="regular-text">';
            echo '<input type="text" name="' . esc_attr(self::OPT_SETTINGS) . '[aria_custom]" value="' . esc_attr($s['aria_custom']) . '" placeholder="' . esc_attr__('Custom aria-label', 'beeclear-smart-link-manager') . '" class="regular-text">';
            echo '</div>';
            echo '</td></tr>';
            ...
            echo '<div class="beeclear-card">';
            echo '<h3 class="ilm-section-title"><span class="dashicons dashicons-visibility" aria-hidden="true"></span>' . esc_html__('Placement & targeting', 'beeclear-smart-link-manager') . '</h3>';
            echo '<table class="form-table" role="presentation">';
            echo '<tr><th>' . esc_html__('Skip elements (INTERNAL)', 'beeclear-smart-link-manager') . '</th><td><input type="text" name="' . esc_attr(self::OPT_SETTINGS) . '[skip_elements_internal]" value="' . esc_attr($s['skip_elements_internal']) . '" class="regular-text" placeholder=""><span class="inline-help">' . esc_html__('Do not inject INTERNAL links inside these HTML tags. Separate with dashes, commas or spaces. Example: h2, h4, li', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '<tr><th>' . esc_html__('Process INTERNAL post types', 'beeclear-smart-link-manager') . '</th><td>';
            echo '<div class="ilm-checkbox-grid">';
            $pts = get_post_types(array('public' => true), 'objects');
            foreach ($pts as $pt) {
                printf(
                    '<label class="ilm-checkbox"><input type="checkbox" name="%1$s[process_post_types][]" value="%2$s"%3$s> %4$s</label>',
            ...
            echo '</div>';
            echo '</td></tr>';
            echo '<tr><th>' . esc_html__('Process INTERNAL links on archives (lists)', 'beeclear-smart-link-manager') . '</th><td><label><input type="checkbox" name="' . esc_attr(self::OPT_SETTINGS) . '[process_on_archives]" value="1" ' . checked($s['process_on_archives'], true, false) . '> ' . esc_html__('Enable (may add load).', 'beeclear-smart-link-manager') . '</label></td></tr>';
            echo '<tr><th>' . esc_html__('Minimum content length to process', 'beeclear-smart-link-manager') . '</th><td><input type="number" min="0" name="' . esc_attr(self::OPT_SETTINGS) . '[min_content_length]" value="' . esc_attr($s['min_content_length']) . '"></td></tr>';
            echo '<tr><th>' . esc_html__('Minimum element length to process', 'beeclear-smart-link-manager') . '</th><td><input type="number" min="0" name="' . esc_attr(self::OPT_SETTINGS) . '[min_element_length]" value="' . esc_attr($s['min_element_length']) . '"><span class="inline-help">' . esc_html__('Skip paragraphs, headings, or list items shorter than this when autolinking.', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '</table>';
            echo '</div>';

            echo '<div class="beeclear-card">';
            echo '<h3 class="ilm-section-title"><span class="dashicons dashicons-filter" aria-hidden="true"></span>' . esc_html__('Limits & caps', 'beeclear-smart-link-manager') . '</h3>';
            echo '<table class="form-table" role="presentation">';
            echo '<tr><th>' . esc_html__('Max INTERNAL links per target (per page)', 'beeclear-smart-link-manager') . '</th><td><input type="number" min="0" name="' . esc_attr(self::OPT_SETTINGS) . '[max_per_target]" value="' . esc_attr($s['max_per_target']) . '"><span class="inline-help">' . esc_html__('0 = unlimited', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '<tr><th>' . esc_html__('Max TOTAL links injected per page', 'beeclear-smart-link-manager') . '</th><td><input type="number" min="0" name="' . esc_attr(self::OPT_SETTINGS) . '[max_total_per_page]" value="' . esc_attr($s['max_total_per_page']) . '"><span class="inline-help">' . esc_html__('0 = unlimited', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '</table>';
            echo '</div>';

            echo '<div class="beeclear-card">';
            echo '<h3 class="ilm-section-title"><span class="dashicons dashicons-update" aria-hidden="true"></span>' . esc_html__('Automation & logs', 'beeclear-smart-link-manager') . '</h3>';
            echo '<table class="form-table" role="presentation">';
            echo '<tr><th>' . esc_html__('Auto-run scan after saving content', 'beeclear-smart-link-manager') . '</th><td><label><input type="checkbox" name="' . esc_attr(self::OPT_SETTINGS) . '[auto_scan_on_save]" value="1" ' . checked(!empty($s['auto_scan_on_save']), true, false) . '> ' . esc_html__('Trigger “Scan site” whenever a supported post/page is added or updated.', 'beeclear-smart-link-manager') . '</label><span class="inline-help">' . esc_html__('Runs a full overview scan after publishing updates.', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '<tr><th>' . esc_html__('Scan speed (pages per second)', 'beeclear-smart-link-manager') . '</th><td><input type="number" min="1" max="20" step="1" name="' . esc_attr(self::OPT_SETTINGS) . '[scan_pages_per_second]" value="' . esc_attr(isset($s['scan_pages_per_second']) ? $s['scan_pages_per_second'] : 5) . '"><span class="inline-help">' . esc_html__('How many pages per second the overview scan should process. Lower values reduce server load. Default: 5.', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '<tr><th>' . esc_html__('Debug: log autolink timing', 'beeclear-smart-link-manager') . '</th><td><label><input type="checkbox" name="' . esc_attr(self::OPT_SETTINGS) . '[log_internal_timing]" value="1" ' . checked(!empty($s['log_internal_timing']), true, false) . '> ' . esc_html__('Console-log extra render time added by internal linking logic.', 'beeclear-smart-link-manager') . '</label><span class="inline-help">' . esc_html__('Shows how many milliseconds were spent in autolinking during this page load (front-end only).', 'beeclear-smart-link-manager') . '</span></td></tr>';
            echo '</table>';
            echo '</div>';

            echo '<div class="beeclear-card">';
            echo '<h3 class="ilm-section-title"><span class="dashicons dashicons-shield" aria-hidden="true"></span>' . esc_html__('Cleanup preferences', 'beeclear-smart-link-manager') . '</h3>';
            echo '<table class="form-table" role="presentation">';
            echo '<tr><th>' . esc_html__('Clean all plugin data on uninstall', 'beeclear-smart-link-manager') . '</th><td><label><input type="checkbox" name="' . esc_attr(self::OPT_SETTINGS) . '[clean_on_uninstall]" value="1" ' . checked($s['clean_on_uninstall'], true, false) . '> ' . esc_html__('Yes, remove data on uninstall', 'beeclear-smart-link-manager') . '</label></td></tr>';
            echo '<tr><th>' . esc_html__('Clean runtime data on deactivation', 'beeclear-smart-link-manager') . '</th><td><label><input type="checkbox" name="' . esc_attr(self::OPT_SETTINGS) . '[clean_on_deactivation]" value="1" ' . checked($s['clean_on_deactivation'], true, false) . '> ' . esc_html__('Clear index & counters when plugin is deactivated', 'beeclear-smart-link-manager') . '</label></td></tr>';
            echo '</table>';
            echo '</div>';
            ...
            echo '<button type="submit" form="beeclear-ilm-settings-form" name="beeclear_ilm_save_settings" value="1" class="button button-primary">' . esc_html__('Save', 'beeclear-smart-link-manager') . '</button>';
            echo '<button type="submit" form="beeclear-ilm-settings-form" name="beeclear_ilm_save_rebuild" value="1" class="button button-secondary">' . esc_html__('Save and rebuild index', 'beeclear-smart-link-manager') . '</button>';
            echo '<form method="post" class="beeclear-inline-action">';
            wp_nonce_field(self::NONCE, self::NONCE);
            echo '<button class="button button-secondary" name="beeclear_ilm_reset_global_settings" value="1" onclick="return confirm(\'' . esc_js(__('Restore all global settings to default values?', 'beeclear-smart-link-manager')) . '\');">' . esc_html__('Restore default settings', 'beeclear-smart-link-manager') . '</button>';
            echo '</form>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            ...
            echo '<div class="beeclear-card">';
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-controls-repeat" aria-hidden="true"></span>' . esc_html__('Actions', 'beeclear-smart-link-manager') . '</h2>';
            echo '<div class="beeclear-actions">';
            echo '<div class="beeclear-actions__row">';
            echo '<form method="post" class="beeclear-inline-action">';
            wp_nonce_field(self::NONCE, self::NONCE);
            echo '<button class="button button-primary" name="beeclear_ilm_reindex_now" value="1">' . esc_html__('Rebuild index now', 'beeclear-smart-link-manager') . '</button>';
            echo '</form>';
            echo '<button type="button" class="button button-secondary" id="beeclear-ilm-start-overview-scan">' . esc_html__('Scan site to refresh overview', 'beeclear-smart-link-manager') . '</button>';
            ...
            echo '<div class="beeclear-progress__label">' . esc_html__('Idle — no scan in progress.', 'beeclear-smart-link-manager') . '</div>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            ...
            echo '<div class="beeclear-card">';
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-visibility" aria-hidden="true"></span>' . esc_html__('Last scan summary', 'beeclear-smart-link-manager') . '</h2>';
            echo '<div id="beeclear-ilm-scan-summary">' . wp_kses_post($this->render_scan_summary_html()) . '</div>';
            echo '</div>';

            echo '<div class="beeclear-card">';
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-admin-tools" aria-hidden="true"></span>' . esc_html__('Maintenance', 'beeclear-smart-link-manager') . '</h2>';
            echo '<div class="beeclear-actions beeclear-actions--stacked">';

            echo '<form method="post" class="beeclear-inline-form">';
            wp_nonce_field(self::NONCE, self::NONCE);
            echo '<label><input type="checkbox" name="confirm" required> ' . esc_html__('Confirm clearing runtime data (index & linkmap).', 'beeclear-smart-link-manager') . '</label>';
            echo '<button class="button button-secondary" name="beeclear_ilm_clear_data" value="1">' . esc_html__('Clear data & rebuild index', 'beeclear-smart-link-manager') . '</button>';
            echo '</form>';
        ...
        private function render_index_summary_html($summary)
        {
            if (!$summary)
                return '';
            $out = '<p>';
            $out .= esc_html__('Total phrases:', 'beeclear-smart-link-manager') . ' <strong>' . intval($summary['total_phrases']) . '</strong> &nbsp; ';
            $out .= esc_html__('Targets with phrases:', 'beeclear-smart-link-manager') . ' <strong>' . intval($summary['total_targets']) . '</strong>';
            $out .= '</p>';
            if (!empty($summary['per_target'])) {
                $out .= '<details><summary>' . esc_html__('Per target breakdown (top 50):', 'beeclear-smart-link-manager') . '</summary>';
                $out .= '<table class="widefat striped" style="margin-top:8px"><thead><tr><th>ID</th><th>' . esc_html__('Title', 'beeclear-smart-link-manager') . '</th><th>' . esc_html__('# phrases', 'beeclear-smart-link-manager') . '</th></tr></thead><tbody>';
                $i = 0;
        ...
        private function render_scan_summary_html()
        {
            ...
            if (empty($summary['completed_at'])) {
                return '<p class="description">' . esc_html__('No scan has been completed yet.', 'beeclear-smart-link-manager') . '</p>';
            }
            ...
            $out .= '<p><strong>' . esc_html__('Last scan', 'beeclear-smart-link-manager') . ':</strong> ' . esc_html($completed) . '</p>';
            $out .= '<ul>';
            $out .= '<li>' . esc_html__('Scanned pages', 'beeclear-smart-link-manager') . ': <strong>' . intval($summary['scanned']) . '</strong></li>';
            $out .= '<li>' . esc_html__('Internal links in index', 'beeclear-smart-link-manager') . ': <strong>' . intval($summary['internal_links']) . '</strong></li>';
            $out .= '</ul>';
            $out .= '</div>';
        ...
        {
            if (empty($entries)) {
                return '<div class="beeclear-card__logs" role="log" aria-live="polite"><p class="description">' . esc_html__('No log entries yet.', 'beeclear-smart-link-manager') . '</p></div>';
            }
        ...
        private function render_activity_log_pagination_html($page, $total_pages)
        {
            ...
            $out = '<div class="beeclear-log-pagination" aria-label="' . esc_attr__('Activity log pagination', 'beeclear-smart-link-manager') . '">';
            $out .= '<button type="button" class="button" data-log-page="' . intval(max(1, $page - 1)) . '"' . $prev_disabled . '>' . esc_html__('Previous', 'beeclear-smart-link-manager') . '</button>';
            /* translators: 1: current page number, 2: total number of pages. */
            $out .= '<span class="beeclear-log-pagination__status">' . esc_html(sprintf(__('Page %1$d of %2$d', 'beeclear-smart-link-manager'), $page, $total_pages)) . '</span>';
            $out .= '<button type="button" class="button" data-log-page="' . intval(min($total_pages, $page + 1)) . '"' . $next_disabled . '>' . esc_html__('Next', 'beeclear-smart-link-manager') . '</button>';
            $out .= '</div>';
        ...
        private function render_overview_table()
        {
            ...
            if ($view === 'targets') {
                if (empty($phrases_per_target)) {
                    echo '<p>' . esc_html__('No phrases defined yet.', 'beeclear-smart-link-manager') . '</p>';
                    return;
                }
                ...
                if (empty($rows)) {
                    echo '<p>' . esc_html__('No links recorded yet.', 'beeclear-smart-link-manager') . '</p>';
                    return;
                }
            } else {
                $rows = $this->build_external_overview_rows();
                if (empty($rows)) {
                    echo '<p>' . esc_html__('No external rules defined yet.', 'beeclear-smart-link-manager') . '</p>';
                    return;
                }
            }
            ...
            if ($search !== '') {
                $rows = array_values(array_filter($rows, function ($row) use ($search_lower, $view) {
                    ...
                    foreach ($fields as $field) {
                        ...
                        if ($field_lower !== '' && strpos($field_lower, $search_lower) !== false)
                            return true;
                    }
                    return false;
                }));
            ...
            $pagination_links = paginate_links(array(
                ...
                'current' => $page,
                'total' => $total_pages,
                'prev_text' => __('« Previous', 'beeclear-smart-link-manager'),
                'next_text' => __('Next »', 'beeclear-smart-link-manager'),
                'add_args' => false,
            ));

            echo '<div class="beeclear-ilm-overview-controls">';
            echo '<div class="beeclear-ilm-view-toggle" role="group" aria-label="' . esc_attr__('Change overview layout', 'beeclear-smart-link-manager') . '">';
            echo '<button class="button button-primary" type="button" disabled>' . esc_html__('By target', 'beeclear-smart-link-manager') . '</button>';
            echo '</div>';
            ...
            echo '<label for="beeclear-ilm-filter" class="screen-reader-text">' . esc_html__('Search', 'beeclear-smart-link-manager') . '</label>';
            echo '<input id="beeclear-ilm-filter" type="search" name="ilm_q" value="' . esc_attr($search) . '" placeholder="' . esc_attr__('Title, URL, phrase, or rule', 'beeclear-smart-link-manager') . '"> ';
            echo '<button class="button" type="submit">' . esc_html__('Filter', 'beeclear-smart-link-manager') . '</button> ';
            if ($search !== '') {
                echo '<a class="button" href="' . esc_url($base_url) . '">' . esc_html__('Reset', 'beeclear-smart-link-manager') . '</a>';
            }
            echo '</form>';
            echo '<div class="beeclear-ilm-overview-meta">';
            echo '<span class="displaying-num">' . intval($total_items) . ' ' . esc_html__('items', 'beeclear-smart-link-manager') . '</span>';
            if ($pagination_links) {
            ...
            if (empty($paged_rows)) {
                echo '<p>' . esc_html__('No results match your search.', 'beeclear-smart-link-manager') . '</p>';
                return;
            }
            ...
            foreach ($paged_rows as $row) {
                ...
                $btn = '<button type="button" class="button button-small beeclear-ilm-popup-btn" data-entry="' . esc_attr($entry_id_attr) . '" data-view="' . esc_attr($view) . '" data-title="' . esc_attr($title) . '">' . esc_html__('Show details', 'beeclear-smart-link-manager') . '</button>';
                echo '<tr>';
                ...
                if ($edit) {
                    echo ' <a href="' . esc_url($edit) . '" class="beeclear-ilm-edit" title="' . esc_attr__('Edit', 'beeclear-smart-link-manager') . '"><span class="dashicons dashicons-edit"></span></a>';
                }
                echo '</td>';
                echo '<td class="col-phrases" data-sort-value="' . (int) $row['phrases_count'] . '">' . (int) $row['phrases_count'] . '</td>';
                if ($view === 'targets') {
                ...
                // Defined phrases column.
                $phrase_chips = '';
                $pt = isset($row['phrase_texts']) ? (array) $row['phrase_texts'] : array();
                if (!empty($pt)) {
                    $chips = array();
                    foreach ($pt as $ptext) {
                        $chips[] = '<span class="beeclear-phrase-chip">' . esc_html($ptext) . '</span>';
                    }
                    $phrase_chips = implode(' ', $chips);
                }
                echo '<td class="col-defined">' . wp_kses_post($phrase_chips) . '</td>';
            ...
            echo '<button type="button" class="beeclear-ilm-popup-close" aria-label="' . esc_attr__('Close', 'beeclear-smart-link-manager') . '">&times;</button>';
            echo '</div>';
            ...
            $popup_script = 'jQuery(function($){'
                ...
                . 'function openPopup(id,view,title,page){currentEntry=id;currentView=view;$title.text(title);$body.html("<p>' . esc_js(__('Loading…', 'beeclear-smart-link-manager')) . '</p>");$pagination.empty();$overlay.removeAttr("hidden");loadPopupPage(page||1);}'
                . 'function closePopup(){$overlay.attr("hidden","hidden");$body.empty();$pagination.empty();currentEntry=null;currentView=null;}'
                . 'function loadPopupPage(page){$.post(ajaxurl,{action:"beeclear_ilm_expand_sources_paginated",id:currentEntry,view:currentView,popup_page:page,_ajax_nonce:popupNonce},function(resp){if(!resp||!resp.success){$body.html("<p>' . esc_js(__('Error loading data.', 'beeclear-smart-link-manager')) . '</p>");return;}$body.html(resp.data.html);renderPagination(resp.data.page,resp.data.total_pages,resp.data.total_items);}).fail(function(){$body.html("<p>' . esc_js(__('Request failed.', 'beeclear-smart-link-manager')) . '</p>");});}'
                . 'function renderPagination(page,totalPages,totalItems){$pagination.empty();if(totalPages<=1){$pagination.append("<span class=\\"beeclear-popup-page-info\\">"+totalItems+" ' . esc_js(__('items', 'beeclear-smart-link-manager')) . '</span>");return;}'
                . 'var info=$("<span>",{class:"beeclear-popup-page-info",text:page+"/"+totalPages+" ("+totalItems+" ' . esc_js(__('items', 'beeclear-smart-link-manager')) . ')"});'
                . 'if(page>1){$pagination.append($("<button>",{text:"«","data-page":1}));$pagination.append($("<button>",{text:"‹","data-page":page-1}));}'
                . 'var startP=Math.max(1,page-2),endP=Math.min(totalPages,page+2);for(var i=startP;i<=endP;i++){var $btn=$("<button>",{text:i,"data-page":i});if(i===page)$btn.addClass("current");$pagination.append($btn);}'
                . 'if(page<totalPages){$pagination.append($("<button>",{text:"›","data-page":page+1}));$pagination.append($("<button>",{text:"»","data-page":totalPages}));}'
                . '$pagination.append(info);}'
                . '$(document).on("click",".beeclear-ilm-popup-btn",function(){var $b=$(this);openPopup($b.data("entry"),$b.data("view"),$b.data("title"),1);});'
                . '$overlay.on("click",".beeclear-ilm-popup-close",closePopup);'
                . '$overlay.on("click",function(e){if(e.target===$overlay[0]&&!$(".beeclear-ilm-context-popup:not([hidden])").length)closePopup();});'
                . '$(document).on("keydown",function(e){if(e.key==="Escape"){if($(".beeclear-ilm-context-popup:not([hidden])").length)return;closePopup();}});'
                . '$pagination.on("click","button[data-page]",function(){loadPopupPage(parseInt($(this).data("page"),10));});'
                . '});';
            ...
            wp_add_inline_script($h, $popup_script, 'after');
            wp_add_inline_script($h, $sort_script, 'after');
        }
        ...
        public function ajax_expand_sources()
        {
            ...
            if (empty($sources)) {
                echo '<em>' . esc_html__('No sources recorded yet.', 'beeclear-smart-link-manager') . '</em>';
                wp_die();
            }
            ...
            foreach ($sources as $sid => $info) {
                ...
                $edit_html = $edit ? ' <a class="beeclear-ilm-edit" href="' . esc_url($edit) . '" title="' . esc_attr__('Edit', 'beeclear-smart-link-manager') . '"><span class="dashicons dashicons-edit"></span></a>' : '';
                    ...
                    foreach ($phrases as $ph => $pc) {
                        ...
                        if (!empty($contexts_by_phrase[$ph])) {
                            foreach ($contexts_by_phrase[$ph] as $ctx) {
                                if (!empty($ctx['manual'])) {
                                    $manual_label = esc_attr__('Manual insertion link', 'beeclear-smart-link-manager');
                                    $part .= ' <span class="beeclear-ilm-manual-badge" title="' . $manual_label . '" aria-label="' . $manual_label . '">'
                                        . '<span class="dashicons dashicons-admin-tools" aria-hidden="true"></span>'
                                        . '<span class="screen-reader-text">' . esc_html__('Manual insertion link', 'beeclear-smart-link-manager') . '</span>'
                                        . '</span>';
                                    break;
                                }
                            }
                ...
                echo '<li class="beeclear-ilm-source-item">' . wp_kses_post($title_html) . wp_kses_post($edit_html) . ($details !== '' ? ' — ' . wp_kses_post($details) : '') . '</li>';
            }
            echo '</ul>';
            wp_die();
        }
        ...
        public function ajax_expand_sources_paginated()
        {
            ...
            ob_start();
            if (empty($paged)) {
                echo '<p><em>' . esc_html__('No data recorded yet. Run the scan first.', 'beeclear-smart-link-manager') . '</em></p>';
            } else {
                $page_col_label = $view === 'sources' ? __('Target', 'beeclear-smart-link-manager') : __('Source', 'beeclear-smart-link-manager');
                echo '<table><thead><tr>';
        ...
        /**
         * Collect flat list of popup items for a given entry.
         */
        ...
        public function ajax_start_overview_scan()
        {
            ...
            if (empty($ids)) {
                delete_option(self::OPT_OVERVIEW_SCAN);
                wp_send_json_error(array('message' => __('No public content found to scan.', 'beeclear-smart-link-manager')));
            }
        ...
        public function render_internal_overview()
        {
            ...
            echo '<div class="beeclear-card">';
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-admin-links" aria-hidden="true"></span>' . esc_html__('Linking overview', 'beeclear-smart-link-manager') . '</h2>';
            echo '<p class="description">' . esc_html__('Review how phrases map to your targets, how many links point to them, and expand entries to inspect exact source snippets.', 'beeclear-smart-link-manager') . '</p>';
            echo '</div>';

            $this->render_overview_table();

            echo '<div class="beeclear-card">';
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-lightbulb" aria-hidden="true"></span>' . esc_html__('How to use this panel', 'beeclear-smart-link-manager') . '</h2>';
            echo '<ol class="ilm-list">';
            echo '<li>' . esc_html__('Scan your site from the Settings tab to refresh counters before reviewing results here.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('Use the “Show” toggle in Sources to reveal the posts/pages where a phrase was auto-linked.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('Click the Edit icon next to a target title to open it and adjust per-post rules or metadata.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('If a phrase needs tweaking, edit it directly on the target post and rebuild the index to apply changes.', 'beeclear-smart-link-manager') . '</li>';
            echo '</ol>';
            echo '</div>';
        ...
        public function render_external()
        {
            ...
            if (isset($_POST['beeclear_ilm_save_external']) && check_admin_referer(self::NONCE, self::NONCE)) {
                ...
                if (!empty($settings['auto_scan_on_external'])) {
                    $pts = !empty($settings['process_post_types']) ? (array) $settings['process_post_types'] : array('post', 'page');
                    /* translators: %d: number of pages queued for the overview scan. */
                    $this->start_overview_scan_now($pts, __('Auto scan started after external rule save: %d pages queued.', 'beeclear-smart-link-manager'), false);
                }
            }
            ...
            echo '<div class="wrap"><h1>' . esc_html__('External linking', 'beeclear-smart-link-manager') . '</h1>';
            echo wp_kses_post($this->render_author_note());
            echo '<div class="beeclear-grid">';
            echo '<div class="ilm-wrap">';

            echo '<div class="beeclear-card">';
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-admin-links" aria-hidden="true"></span>' . esc_html__('External rules', 'beeclear-smart-link-manager') . '</h2>';
            ...
            echo '<form method="post"><table class="widefat striped" id="beeclear-ilm-ext-table"><thead><tr>' .
                ...
                '<th>' . esc_html__('Post types / Exclusions', 'beeclear-smart-link-manager') . '</th>' .
                '<th>' . esc_html__('Actions', 'beeclear-smart-link-manager') . '</th>' .
                '</tr></thead><tbody>';
            ...
            foreach ($rules as $r) {
                ...
                echo '<td class="cell-phrase">' .
                    ...
                    '<label class="ext-destination-label" for="' . esc_attr($url_id) . '">' . esc_html__('Destination URL', 'beeclear-smart-link-manager') . '</label>' .
                    ...
                    '<label class="ext-phrase-label" for="' . esc_attr($phrase_id) . '">' . esc_html__('Phrase or regex', 'beeclear-smart-link-manager') . '</label>' .
                    '<input type="text" id="' . esc_attr($phrase_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][phrase]" class="regular-text" value="' . esc_attr($phrase) . '" placeholder="' . esc_attr__('Phrase or regex', 'beeclear-smart-link-manager') . '">' .
                    '</div>' .
                    '<div class="flags">' .
                    '<label><input type="checkbox" class="ext-regex" name="beeclear_ilm_ext[' . intval($i_attr) . '][regex]" value="1" ' . checked($regex, true, false) . '> ' . esc_html__('Regex', 'beeclear-smart-link-manager') . '</label> ' .
                    '<label><input type="checkbox" class="ext-case" name="beeclear_ilm_ext[' . intval($i_attr) . '][case]" value="1" ' . checked($case, true, false) . ($regex ? ' disabled' : '') . '> ' . esc_html__('Case-sensitive', 'beeclear-smart-link-manager') . '</label>' .
                    '</div>' .
                    '<div class="ext-field ext-context">' .
                    '<label class="ext-context-label" for="' . esc_attr($context_id) . '">' . esc_html__('Context words or regex', 'beeclear-smart-link-manager') . '</label>' .
                    '<input type="text" id="' . esc_attr($context_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][context]" class="regular-text" value="' . esc_attr($context_display) . '" placeholder="' . esc_attr__('Additional words required in the same element', 'beeclear-smart-link-manager') . '">' .
                    '<div class="flags">' .
                    '<label><input type="checkbox" class="ext-context-regex" name="beeclear_ilm_ext[' . intval($i_attr) . '][context_regex]" value="1" ' . checked($context_regex, true, false) . '> ' . esc_html__('Regex', 'beeclear-smart-link-manager') . '</label> ' .
                    '<label><input type="checkbox" class="ext-context-case" name="beeclear_ilm_ext[' . intval($i_attr) . '][context_case]" value="1" ' . checked($context_case, true, false) . ($context_regex ? ' disabled' : '') . '> ' . esc_html__('Case-sensitive', 'beeclear-smart-link-manager') . '</label>' .
                    '</div>' .
                    '<p class="description">' . esc_html__('Supports token syntax (non-regex). Separate multiple entries with commas.', 'beeclear-smart-link-manager') . '</p>' .
                    '</div>' .
                    '</td>';
                echo '<td class="cell-attrs"><div class="attr-rows">' .
                    ...
                    '<div class="ar"><label class="ar-label" for="' . esc_attr($title_mode_id) . '">' . esc_html__('Title', 'beeclear-smart-link-manager') . '</label><div class="ar-field"><div class="inline-field"><select id="' . esc_attr($title_mode_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][title_mode]">' .
                    wp_kses_post($this->options_html(array('none', 'phrase', 'custom'), $title_mode)) . '</select></div>' .
                    '<div class="inline-field"><label class="screen-reader-text" for="' . esc_attr($title_custom_id) . '">' . esc_html__('Custom title', 'beeclear-smart-link-manager') . '</label><input type="text" id="' . esc_attr($title_custom_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][title_custom]" class="regular-text" value="' . esc_attr($title_custom) . '" placeholder="' . esc_attr__('Custom title', 'beeclear-smart-link-manager') . '"></div></div></div>' .
                    '<div class="ar"><label class="ar-label" for="' . esc_attr($aria_mode_id) . '">' . esc_html__('Aria-label', 'beeclear-smart-link-manager') . '</label><div class="ar-field"><div class="inline-field"><select id="' . esc_attr($aria_mode_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][aria_mode]">' .
                    wp_kses_post($this->options_html(array('none', 'phrase', 'custom'), $aria_mode)) . '</select></div>' .
                    '<div class="inline-field"><label class="screen-reader-text" for="' . esc_attr($aria_custom_id) . '">' . esc_html__('Custom aria-label', 'beeclear-smart-link-manager') . '</label><input type="text" id="' . esc_attr($aria_custom_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][aria_custom]" class="regular-text" value="' . esc_attr($aria_custom) . '" placeholder="' . esc_attr__('Custom aria-label', 'beeclear-smart-link-manager') . '"></div></div></div>' .
                ...
                echo '<td class="cell-types">' .
                    ...
                    '<div class="max-per-page-field"><label class="max-label" for="' . esc_attr($max_id) . '">' . esc_html__('Max/page', 'beeclear-smart-link-manager') . '</label><input type="number" min="0" id="' . esc_attr($max_id) . '" name="beeclear_ilm_ext[' . intval($i_attr) . '][max_per_page]" value="' . esc_attr((int) $max_per_page) . '"> <span class="desc">' . esc_html__('0 = unlimited', 'beeclear-smart-link-manager') . '</span></div>' .
                    ...
                    '<label class="field-label" for="beeclear-ilm-ext-' . intval($i_attr) . '-exclude">' . esc_html__('Exclude by post ID', 'beeclear-smart-link-manager') . '</label>' .
                    ...
                    '</div>' .
                    '<div class="field-stack">' .
                    '<label class="field-label" for="beeclear-ilm-ext-' . intval($i_attr) . '-allowed">' . esc_html__('Allowed elements (overrides global skip)', 'beeclear-smart-link-manager') . '</label>' .
                    ...
                    '<p class="description">' . esc_html__('Comma-separated tag names. Leave empty to follow global “Skip elements (EXTERNAL)” setting.', 'beeclear-smart-link-manager') . '</p>' .
                    '</div>' .
                    '</div>' .
                    '</td>';
                echo '<td class="cell-actions"><a href="#" class="button ext-delete">' . esc_html__('Remove', 'beeclear-smart-link-manager') . '</a></td>';
                echo '</tr>';
                $i++;
            }

            echo '</tbody></table>';
            echo '<p><a href="#" class="button" id="beeclear-ilm-ext-add">' . esc_html__('Add external rule', 'beeclear-smart-link-manager') . '</a></p>';
            wp_nonce_field(self::NONCE, self::NONCE);
            echo '<p><button class="button button-primary" name="beeclear_ilm_save_external" value="1">' . esc_html__('Save rules', 'beeclear-smart-link-manager') . '</button></p>';
            echo '</form>';
            echo '</div>';
            ...
            echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-lightbulb" aria-hidden="true"></span>' . esc_html__('How to use this panel', 'beeclear-smart-link-manager') . '</h2>';
            echo '<ol class="ilm-list">';
            echo '<li>' . esc_html__('Add or edit rows to pair a Phrase with a destination URL. Use Phrase helpers like [words] or [string] to cover variable text fragments.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('Toggle Regex or Case-sensitive when needed; regex disables case switching.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('Adjust rel/title/aria/class attributes, per-page limits, and restrict rules to selected post types.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('Click “Save rules” to store changes and apply them across your site.', 'beeclear-smart-link-manager') . '</li>';
            echo '<li>' . esc_html__('Review the overview list below to confirm active rules and their targets.', 'beeclear-smart-link-manager') . '</li>';
            echo '</ol>';
            echo '</div>';

	            // External types template (avoid echoing <script> in PHP)
	            $ext_types_template = ''
	                . '<div class="types-stack">'
	                . '<div class="max-per-page-field"><label class="max-label" for="beeclear-ilm-ext-__IDX__-max">' . esc_html__('Max/page', 'beeclear-smart-link-manager') . '</label><input type="number" min="0" id="beeclear-ilm-ext-__IDX__-max" name="beeclear_ilm_ext[__IDX__][max_per_page]" value="1"> <span class="desc">' . esc_html__('0 = unlimited', 'beeclear-smart-link-manager') . '</span></div>'
	                . '<div class="types-checklist">' . wp_kses_post($this->post_types_checklist_html('beeclear_ilm_ext[__IDX__][types][]', array(), $pts, true)) . '</div>'
	                . '<div class="field-stack">'
	                . '<label class="field-label" for="beeclear-ilm-ext-__IDX__-exclude">' . esc_html__('Exclude by post ID', 'beeclear-smart-link-manager') . '</label>'
	                ...
	                . '</div>'
	                . '<div class="field-stack">'
	                . '<label class="field-label" for="beeclear-ilm-ext-__IDX__-allowed">' . esc_html__('Allowed elements (overrides global skip)', 'beeclear-smart-link-manager') . '</label>'
	            ...
	            $ext_template_handle = 'beeclear-ilm-admin-ext-template';
	            wp_register_script($ext_template_handle, '', array('jquery'), self::VERSION, true);
	            wp_enqueue_script($ext_template_handle);
	            wp_add_inline_script(
	                $ext_template_handle,
	                'jQuery(function($){\n'
	                . '  if (!document.getElementById("beeclear-ilm-ext-types-template")) {\n'
	                . '    var el = document.createElement("script");\n'
	                . '    el.type = "text/html";\n'
	                . '    el.id = "beeclear-ilm-ext-types-template";\n'
	                . '    el.text = ' . wp_json_encode($ext_types_template) . ';\n'
            ...
            if (!empty($rules)) {
                echo '<div class="beeclear-card">';
                echo '<h2 class="ilm-section-title"><span class="dashicons dashicons-visibility" aria-hidden="true"></span>' . esc_html__('External rules overview', 'beeclear-smart-link-manager') . '</h2>';
                echo '<ol class="ilm-list">';
                foreach ($rules as $r) {
                    $badge = !empty($r['regex']) ? 'regex' : (!empty($r['case']) ? 'case' : ($this->contains_tokens($r['phrase'] ?? '') ? 'tokens' : ''));
                    $types_raw = empty($r['types']) ? __('all types', 'beeclear-smart-link-manager') : implode(', ', (array) $r['types']);
        ...
        private function options_html($options, $selected)
        {
            ...
            foreach ($options as $opt) {
                $out .= '<option value="' . esc_attr($opt) . '"' . selected($selected, $opt, false) . '>' . esc_html($opt) . '</option>';
            }
            return $out;
        }
        private function post_types_checklist_html($name, $selected, $pts, $template = false)
        {
            ...
            foreach ($pts as $pt) {
                ...
                $out .= sprintf(
                    '<label class="ilm-types-checkbox"><input type="checkbox" name="%1$s" value="%2$s"%3$s> %4$s</label>',
                    esc_attr($name),
                    esc_attr($pt->name),
                    $checked_attr,
                    esc_html($pt->labels->singular_name)
        ...
        public function render_impex()
        {
            ...
            if (isset($_POST['beeclear_ilm_import']) && check_admin_referer(self::NONCE, self::NONCE)) {
                ...
                if (is_array($data)) {
                    if (isset($data['settings']))
                        update_option(self::OPT_SETTINGS, $this->sanitize_settings($data['settings']), false);
                    if (isset($data['external']) && is_array($data['external']))
                        update_option(self::OPT_EXT_RULES, $this->sanitize_external_rules($data['external']), false);
            ...
            <div class="wrap">
				...
				<h1>
					<?php esc_html_e('Import/Export', 'beeclear-smart-link-manager'); ?>
				</h1>
				<?php echo wp_kses_post($this->render_author_note()); ?>
				<div class="ilm-wrap">
                    <div class="beeclear-card">
                        <h2>
                            <?php esc_html_e('Export', 'beeclear-smart-link-manager'); ?>
                        </h2>
                        <textarea rows="16" class="large-text code" readonly
                            id="beeclear-ilm-export-json"><?php echo esc_textarea($json); ?></textarea>
                        <p><button type="button" class="button" id="beeclear-ilm-export-download">
                                <?php esc_html_e('Download JSON file', 'beeclear-smart-link-manager'); ?>
                            </button></p>
                    </div>

                    <div class="beeclear-card">
                        <h2>
                            <?php esc_html_e('Import', 'beeclear-smart-link-manager'); ?>
                        </h2>
                        <form method="post">
                            <?php wp_nonce_field(self::NONCE, self::NONCE); ?>
                            <p><input type="file" id="beeclear-ilm-import-file" accept="application/json,.json"></p>
                            <textarea name="beeclear_ilm_json" rows="10" class="large-text code" id="beeclear-ilm-import-json"
                                placeholder="<?php esc_attr_e('Paste JSON here…', 'beeclear-smart-link-manager'); ?>"></textarea>
                            <p><button class="button button-primary" name="beeclear_ilm_import" value="1">
                                    <?php esc_html_e('Import', 'beeclear-smart-link-manager'); ?>
                                </button></p>
                        </form>
                    </div>
        ...
        public function register_admin_columns()
        {
            ...
            foreach ($pts as $pt) {
                add_filter("manage_{$pt}_posts_columns", function ($cols) {
                    if (!is_array($cols))
                        $cols = array();
                    $cols['beeclear_ilm'] = __('Internal Links (BeeClear)', 'beeclear-smart-link-manager');
                    return $cols;
                });
                add_action("manage_{$pt}_posts_custom_column", function ($col, $post_id) {
                    ...
                    if (empty($rules) || !is_array($rules)) {
                        echo '<span style="opacity:.6">' . esc_html__('— none —', 'beeclear-smart-link-manager') . '</span>';
                    } else {
                        $count = count($rules);
                        $preview_items = array();
